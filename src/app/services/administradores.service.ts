...
// Servicio para actualizar un administrador existente
public actualizarAdmin(data: any): Observable<any> {
  return this.http.put<any>(`${environment.url_api}/admin/${data.clave_admin}`, data, httpOptions);
}
...
public getAdminByID(idUser: Number){
  return this.http.get<any>(`${environment.url_api}/admin/?id=${idUser}`,httpOptions);//cadena compuesta, pq imprime valores
}
...
public getTotalUsuarios(){
  var token= this.facadeService.getSessionToken();
  var headers= new HttpHeaders({'Content-Type': 'aplication/json', 'Authorization': 'Bearer '+token});
